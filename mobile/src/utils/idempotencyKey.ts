/**
 * Compute idempotency_key = sha256(employeeId + type + workDate)
 * Used by offline-sync flow to guarantee dedup when retrying.
 *
 * Note: for RN we use a pure-JS SHA-256 to avoid native dep.
 */
...
export function computeIdempotencyKey(
  employeeId: string,
  eventType: 'auto_checkin' | 'auto_checkout' | 'manual_checkin',
  workDate: string,
): string {
  return toHex(sha256Bytes(`${employeeId}|${eventType}|${workDate}`));
}
